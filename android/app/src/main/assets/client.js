...
async function fetchHomepageBootstrapCatalog() {
  ...
  const response = await fetchWithTimeout(`${HOMEPAGE_BOOTSTRAP_ENDPOINT}?v=530`, { cache: "force-cache" }, 2500);
...
function renderCarousel() {
  ...
  if (!items.length) {
    ...
    if (carouselBackdropImage) {
      carouselBackdropImage.src = "hero-backdrop-placeholder.webp?v=530";
      carouselBackdropImage.removeAttribute("srcset");
      carouselBackdropImage.classList.remove("has-banner");
    }
...
function startUpdateManagerWhenIdle() {
  const start = async () => {
    try {
      if (!window.UpdateManager) await loadExternalScript("/update-manager.js?v=530");
