...
async function fetchHomepageBootstrapCatalog() {
  ...
  const response = await fetchWithTimeout(`${HOMEPAGE_BOOTSTRAP_ENDPOINT}?v=692`, { cache: "force-cache" }, 2500);
  ...
  if (!items.length) {
    ...
    if (!heroMemoActive) {
      ...
      if (carouselBackdropImage) {
        carouselBackdropImage.src = "hero-backdrop-placeholder.webp?v=692";
        carouselBackdropImage.removeAttribute("srcset");
        carouselBackdropImage.classList.remove("has-banner");
      }
...
function startUpdateManagerWhenIdle() {
  const start = async () => {
    try {
      if (!window.UpdateManager) await loadExternalScript("/update-manager.js?v=692");
