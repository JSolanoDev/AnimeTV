...
test("local-only retired files cannot change the production catalog total", async () => {
  ...
  for (const hasRetiredFile of [false, true]) {
    ...
    const c = vm.createContext({
      ...
      applyUnderHentaiPortraitArtwork: item => item,
...
test("shared metadata cache lifetimes match the production policy", () => {
  ...
  assert.match(server, /JIKAN_FULL_CACHE[\s\S]*?s-maxage=86400/);
  assert.match(server, /JIKAN_EPISODES_CACHE[\s\S]*?s-maxage=21600/);
