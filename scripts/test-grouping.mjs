...
const { mergeShows: mergeServerShows, splitDescriptionForTranslation, cleanServerDescription, compactCatalogPayload, findArtworkDescription } = require("../animetv-server.js");
...
const compactSynopsis = compactCatalogPayload({ items: [{ id: "one", description: longSynopsis }] }).items[0].description;
check("Home catalog sends a short synopsis preview", compactSynopsis.length <= 321 && compactSynopsis.endsWith("…"));
check("Bundled artwork keeps the full detail synopsis", findArtworkDescription({ one: { meta: { description: longSynopsis } } }, "one") === longSynopsis);
check("Exact identity fallback finds a full synopsis", findArtworkDescription({ one: { anilistId: 5, meta: {} }, two: { anilistId: 5, meta: { description: longSynopsis } } }, "one", "5") === longSynopsis);
