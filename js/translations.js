const TRANSLATIONS = {
  en: {
    navHome: "Home",
    navSearch: "Search",
    navSchedule: "Schedule",
    navReleases: "Releases",
    navFavorites: "Favorites",
    navSources: "Sources",
    navSettings: "Settings",
    featuredNow: "Recent Releases",
    loadingAnime: "Loading anime...",
    fetchingAnime: "Fetching today's anime from AniList and Jikan.",
    loading: "Loading",
    play: "Play",
    cast: "Cast",
    favorite: "Favorite",
    favorited: "Favorited",
    latestEpisodes: "Latest Episodes",
    animeLibrary: "Anime Library",
    weeklySchedule: "Weekly Schedule",
    scheduleCalendar: "Airing calendar",
    scheduleAllDays: "All days",
    scheduleToday: "Today",
    scheduleEpisodes: "episodes",
    scheduleAiring: "Airing",
    scheduleTimeTba: "Time TBA",
    scheduleNoEpisodes: "No new episodes are scheduled for this day.",
    scheduleLocalTime: "Local time",
    anipubSummary: "External playback catalog",
    favorites: "Favorites",
    emptyFavorites: "Open a title and press Favorite to keep it here.",
    sourcesAddons: "Sources & Addons",
    sourceSummaryDefault: "Add local or online catalog endpoints that return normalized anime JSON.",
    settingsTitle: "Settings",
    settingsSummary: "Change language, playback defaults, and TV layout preferences.",
    searchShort: "Search...",
    searchLong: "Search anime, seasons, episodes...",
    searchAnime: "Search anime...",
    searchAniPub: "Search AniPub...",
    all: "All",
    watchPlaceholder: "Choose an episode to load available playback servers.",
    descriptionLoading: "Loading description...",
    seeMore: "See more",
    seeLess: "See less",
    language: "Language",
    appLanguage: "App language",
    english: "English",
    spanish: "Spanish",
    playback: "Playback",
    defaultAudio: "Default audio",
    defaultSubtitles: "Default subtitles",
    japaneseAudio: "Japanese Audio",
    spanishAudio: "Spanish Audio",
    englishAudio: "English Audio",
    spanishSubtitles: "Spanish Subtitles",
    englishSubtitles: "English Subtitles",
    translatedSpanishSubtitles: "Spanish (translated)",
    noSubtitles: "No Subtitles",
    playerDefaults: "JP audio / ES subtitles",
    refreshPlayer: "Refresh",
    backToDetails: "Back",
    action: "Action",
    comedy: "Comedy",
    fantasy: "Fantasy",
    romance: "Romance",
    layout: "Layout",
    compactSidebar: "Compact sidebar",
    expandedSidebar: "Expanded sidebar",
    cache: "Cache",
    clearCache: "Clear metadata cache",
    cacheCleared: "Cache cleared",
    save: "Save",
    settingsSaved: "Settings saved",
    appearance: "Appearance",
    theme: "Theme",
    darkMode: "Dark",
    lightMode: "Light",
    systemMode: "System",
    behavior: "Experience",
    motion: "Animations",
    motionOn: "Cinematic",
    motionOff: "Reduced",
    tvFocus: "TV focus glow",
    on: "On",
    off: "Off",
    autoplayHero: "Auto carousel",
    installKicker: "ZENKAITV APP",
    installTitle: "Install ZenkaiTV",
    installMessage: "Open it faster from your home screen.",
    installAction: "Install",
    installSteps: "Install steps",
    installGotIt: "Got it",
    installIosHint: "In Safari, tap Share, then Add to Home Screen.",
    installDismiss: "Dismiss install recommendation",
    installNever: "Do not show again",
    installSettingsGroup: "App",
    installSettingsTitle: "Install ZenkaiTV",
    installSettingsMessage: "Add ZenkaiTV to this device for faster access.",
    installSettingsMessageHidden: "Install here whenever you are ready. Recommendations are hidden.",
    installInstalled: "Installed",
    installAccepted: "Installation started",
    installUnavailable: "Installation is not available in this browser yet.",
    dataTools: "Data tools",
    resetSettings: "Reset settings",
    terms: "Terms",
    privacy: "Privacy"
  },
  es: {
    navHome: "Inicio",
    navSearch: "Buscar",
    navSchedule: "Programación",
    navReleases: "Estrenos",
    navFavorites: "Favoritos",
    navSources: "Fuentes",
    navSettings: "Ajustes",
    featuredNow: "Estrenos recientes",
    loadingAnime: "Cargando anime...",
    fetchingAnime: "Buscando anime de hoy en AniList y Jikan.",
    loading: "Cargando",
    play: "Reproducir",
    cast: "Enviar",
    favorite: "Favorito",
    favorited: "En favoritos",
    latestEpisodes: "Últimos episodios",
    animeLibrary: "Biblioteca de anime",
    weeklySchedule: "Programación semanal",
    scheduleCalendar: "Calendario de emisión",
    scheduleAllDays: "Todos los días",
    scheduleToday: "Hoy",
    scheduleEpisodes: "episodios",
    scheduleAiring: "En emisión",
    scheduleTimeTba: "Hora por confirmar",
    scheduleNoEpisodes: "No hay episodios nuevos programados para este día.",
    scheduleLocalTime: "Hora local",
    anipubSummary: "Catálogo con reproducción integrada",
    favorites: "Favoritos",
    emptyFavorites: "Abre un título y pulsa Favorito para guardarlo aquí.",
    sourcesAddons: "Fuentes y addons",
    sourceSummaryDefault: "Añade endpoints locales u online que devuelvan JSON de anime normalizado.",
    settingsTitle: "Ajustes",
    settingsSummary: "Cambia idioma, reproducción y preferencias para TV.",
    searchShort: "Buscar...",
    searchLong: "Buscar anime, temporadas, episodios...",
    searchAnime: "Buscar anime...",
    searchAniPub: "Buscar AniPub...",
    all: "Todo",
    watchPlaceholder: "Los datos vienen de AniList y Jikan. Añade tu propio enlace de video para reproducir.",
    descriptionLoading: "Cargando descripción...",
    seeMore: "Ver más",
    seeLess: "Ver menos",
    language: "Idioma",
    appLanguage: "Idioma de la app",
    english: "Inglés",
    spanish: "Español",
    playback: "Reproducción",
    defaultAudio: "Audio predeterminado",
    defaultSubtitles: "Subtítulos predeterminados",
    japaneseAudio: "Audio japonés",
    spanishAudio: "Audio español",
    englishAudio: "Audio inglés",
    spanishSubtitles: "Subtítulos en español",
    englishSubtitles: "Subtítulos en inglés",
    translatedSpanishSubtitles: "Español traducido",
    noSubtitles: "Sin subtítulos",
    playerDefaults: "Audio JP / subtítulos ES",
    refreshPlayer: "Actualizar",
    backToDetails: "Volver",
    action: "Acción",
    comedy: "Comedia",
    fantasy: "Fantasía",
    romance: "Romance",
    layout: "Diseño",
    compactSidebar: "Barra compacta",
    expandedSidebar: "Barra expandida",
    cache: "Caché",
    clearCache: "Limpiar caché",
    cacheCleared: "Caché limpiada",
    save: "Guardar",
    settingsSaved: "Ajustes guardados",
    appearance: "Apariencia",
    theme: "Tema",
    darkMode: "Oscuro",
    lightMode: "Claro",
    systemMode: "Sistema",
    behavior: "Experiencia",
    motion: "Animaciones",
    motionOn: "Cinemáticas",
    motionOff: "Reducidas",
    tvFocus: "Brillo de enfoque TV",
    on: "Activado",
    off: "Desactivado",
    autoplayHero: "Carrusel automático",
    installKicker: "APP ZENKAITV",
    installTitle: "Instala ZenkaiTV",
    installMessage: "Ábrela más rápido desde tu pantalla de inicio.",
    installAction: "Instalar",
    installSteps: "Ver pasos",
    installGotIt: "Entendido",
    installIosHint: "En Safari, pulsa Compartir y luego Añadir a pantalla de inicio.",
    installDismiss: "Cerrar recomendación de instalación",
    installNever: "No volver a mostrar",
    installSettingsGroup: "Aplicación",
    installSettingsTitle: "Instalar ZenkaiTV",
    installSettingsMessage: "Añade ZenkaiTV a este dispositivo para acceder más rápido.",
    installSettingsMessageHidden: "Instálala aquí cuando quieras. Las recomendaciones están ocultas.",
    installInstalled: "Instalada",
    installAccepted: "Instalación iniciada",
    installUnavailable: "La instalación aún no está disponible en este navegador.",
    dataTools: "Herramientas de datos",
    resetSettings: "Restablecer ajustes",
    terms: "Terms",
    privacy: "Privacy"
  }
};
